	...
	public static void listen() {
		...
		while ((line = s.next())!=null) {
			...
			// clones deck
				...
				if (parts.length!=4) { error("Syntax error!"); continue; }
				int result = DeckLoader.cloneDeck(Integer.parseInt(parts[1]), Integer.parseInt(parts[2]), parts[3]);
				if (result==-1) error("Deck does not exist!");
				else if (result==-2) error("Failed to generate ID!");
				else if (result==-3) error("Unknown error!");
				else info("Succesfully cloned deck. (new ID: "+result+")");
			}
			// deletes deck
			else if(line.startsWith("deletedeck_")){
				if (parts.length!=2) { error("Syntax error!"); continue; }
				int result = DeckLoader.deleteDeck(Integer.parseInt(parts[1]));
				if (result==-1) error("Deck does not exist!");
				else if (result==1) info("Succesfully deleted deck. (ID: "+result+")");
			}
			send("");
		}
